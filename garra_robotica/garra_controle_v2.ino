...
// ==========================================
// 3. INTERFACE WEB (COM DIAGNÓSTICO)
// ==========================================
...
<html lang="pt-BR">
  ...
  <title>Robô Driver Universal</title>
    <style>
    ...
    #statusgp { color: #555; font-weight: bold; margin-bottom: 5px; font-size: 14px; }
    #debug { font-family: monospace; font-size: 12px; color: #333; background: #ddd; padding: 5px; border-radius: 5px; margin: 10px auto; width: 90%; word-wrap: break-word;}
  ...
  <body>
    ...
    <div id="statusgp">Mova o Joystick...</div>
    
    <div id="debug">Aguardando dados...</div>
    ...
    <script>
      ...
      window.addEventListener("gamepadconnected", function(e) {
        document.getElementById("statusgp").innerHTML = "🎮 Conectado: " + e.gamepad.id;
        document.getElementById("statusgp").className = "connected";
        loopGamepad(); 
      });
      ...
      function loopGamepad() {
        ...
        if (gp) {
          ...
          // DIAGNÓSTICO VISUAL (Para você ver se o driver está mandando Axis ou Button)
          // Mostra os 2 primeiros eixos e se algum D-pad está apertado
          var debugText = "Eixos: " + gp.axes[0].toFixed(1) + " | " + gp.axes[1].toFixed(1);
          if (gp.axes.length > 9) debugText += " | Hat: " + gp.axes[9].toFixed(1);
          document.getElementById("debug").innerText = debugText;

          if (agora - lastUpdate > 80) { 
            
            // --- LÓGICA UNIVERSAL (Lê tudo ao mesmo tempo) ---
            
            // 1. BASE (Esquerda / Direita)
            // Aceita: Analógico Esq (Eixo 0), D-Pad Digital (Btn 14/15) ou Hat Switch (Eixo 0 ou 4 ou 9 dependendo do driver)
            if (gp.axes[0] < -0.5 || (gp.buttons[14] && gp.buttons[14].pressed) || (gp.axes[9] && gp.axes[9] == -1)) {
                sendCommand('Base', 'LEFT');
            }
            if (gp.axes[0] > 0.5 || (gp.buttons[15] && gp.buttons[15].pressed) || (gp.axes[9] && gp.axes[9] == 1)) {
                sendCommand('Base', 'RIGHT');
            }

            // 2. OMBRO (Cima / Baixo)
            // Aceita: Analógico Esq (Eixo 1), D-Pad Digital (Btn 12/13) ou Hat Switch Y
            if (gp.axes[1] < -0.5 || (gp.buttons[12] && gp.buttons[12].pressed) || (gp.axes[10] && gp.axes[10] == -1)) {
                sendCommand('Shoulder', 'UP');
            }
            if (gp.axes[1] > 0.5 || (gp.buttons[13] && gp.buttons[13].pressed) || (gp.axes[10] && gp.axes[10] == 1)) {
                sendCommand('Shoulder', 'DOWN');
            }

            // 3. BOTÕES (Você disse que já funcionam, mantive o mapeamento padrão)
            if (gp.buttons[3] && gp.buttons[3].pressed) sendCommand('Elbow', 'UP');    // Y
            if (gp.buttons[0] && gp.buttons[0].pressed) sendCommand('Elbow', 'DOWN');  // A
            if (gp.buttons[2] && gp.buttons[2].pressed) sendCommand('Gripper', 'CLOSE'); // X
            if (gp.buttons[1] && gp.buttons[1].pressed) sendCommand('Gripper', 'OPEN');  // B
...
void processarManual(String parte, String acao) {
  if (modoDemoAtivo) modoDemoAtivo = false;
...
void onWebSocketEvent(AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type, void *arg, uint8_t *data, size_t len) {                      
  if (type == WS_EVT_DATA) {
    ...
    if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
      ...
      if (key == "Demo") {
        if (val == "ON") { modoDemoAtivo = true; passoDemo = 0; } 
        else { modoDemoAtivo = false; }
      } 
      else if (key == "Home") {
        modoDemoAtivo = false; 
